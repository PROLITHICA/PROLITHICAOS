...
@Component({
  ...
  template: `
    <div>
      ...
      <svg viewBox="0 0 620 200" width="100%" height="200" aria-hidden="true">
        ...
        <line x1="40" y1="170" x2="610" y2="170" stroke="#d8d8d8" />

        <g font-size="10" fill="#9a9a9a" font-family="Roboto, sans-serif">
        ...
        @for (s of series(); track $index) {
          @if (s.area) { <polygon [attr.points]="s.area" [attr.fill]="s.areaFill || '#f4f4f4'" /> }
        }
        @for (s of series(); track $index) {
          <polyline [attr.points]="s.points" fill="none" [attr.stroke]="s.color" [attr.stroke-width]="s.width || 2" />
        }
        @for (s of series(); track $index) {
          <g [attr.fill]="s.color">
        ...
        @if (nowX() !== null) {
          <line [attr.x1]="nowX()" y1="20" [attr.x2]="nowX()" y2="170" stroke="#111111" stroke-width="1" stroke-dasharray="4 3" />
          <text [attr.x]="(nowX() || 0) + 6" y="34" font-size="10" fill="#111111" font-family="Roboto, sans-serif">{{ nowLabel() }}</text>
        }
  ...
  styles: [`
    :host { display: block; }
    .legend { display: flex; flex-wrap: wrap; gap: 6px 14px; font-size: 11px; color: #6b6b6b; justify-content: flex-end; }
